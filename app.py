...
def retryable(fn, tries=3, base_sleep=0.4, max_sleep=2.0):
    last = None
...
def get_worksheet(sheet_id: str, tab: str):
    sh = open_sheet(sheet_id)
...
def load_config_tables() -> Dict[str, pd.DataFrame]:
    ...
    areas_values = retryable(lambda: get_worksheet(CONFIG_SHEET_ID, ws_areas).get_all_records())
    itens_values = retryable(lambda: get_worksheet(CONFIG_SHEET_ID, ws_itens).get_all_records())

    areas = pd.DataFrame(areas_values)
    itens = pd.DataFrame(itens_values)
    ...
    if itens.empty:
        ...
        if "dia_semana" not in itens.columns:
            if len(raw_cols) >= 16:
                col_p = raw_cols[15]
                itens["dia_semana"] = itens[col_p].astype(str)
            else:
                itens["dia_semana"] = ""
...
def load_users_table() -> pd.DataFrame:
    ...
    users_values = retryable(lambda: get_worksheet(RULES_SHEET_ID, ws_users).get_all_records())
    df = pd.DataFrame(users_values)
...
def load_events_last(last_rows: int = 800) -> pd.DataFrame:
    require_ids()
    ...
    header = values[0]
    body = values[-last_rows:] if len(values) > last_rows + 1 else values[1:]
...
def main():
    ...
    events_df = load_events_last(last_rows=800)
